...
by applying the filtering steps described also in \citep{Castelo:2009fk}. You can
...
<<getthenet, echo=TRUE, results=verbatim>>=
set.seed(123)
rndcor <- qpUnifRndCor(100, featureNames(subset.gds680.eset))
...
A <- qpGraph(avgnrr.estimates, threshold=thr)
@
...
\verb+qpgraph+ package:

<<getthenet, echo=TRUE, results=verbatim>>=
qpCliqueNumber(A, verbose=FALSE)
@
...
<<getthenet, echo=TRUE, results=verbatim>>=
pac.estimates <- qpPAC(subset.gds680.eset, A, verbose=FALSE)
@
In order to display the final reverse engineered transcriptional regulatory network
from this 50\%-precision qp-graph we need to obtain it as a \verb+graphNEL+ object
which is returned by the \verb+qpGraph()+ function as an undirected graph and then
transform it into a directed graph with arrows pointing from TFs to
their targets:
...
legend(bb[1]*0.2,bb[2]*1.0,c("RegulonDB +","RegulonDB -","Novel +", "Novel -"),
