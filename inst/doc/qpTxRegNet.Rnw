...
This vignette describes how to use the package \verb+qpgraph+ in order to reverse
...
library(qpgraph)
@
Within the \verb+qpgraph+ package there is a data file called \verb+EcoliOxygen+
...
filtering steps described in \citep{Castelo:2008uq} and \verb+gds680.eset+ is an
...
by applying the filtering steps described also in \citep{Castelo:2008uq}. You can
...
variability measured by the interquartile range (IQR). In the \verb+qpgraph+
...
PCCs for all gene pairs with the function \verb+qpPCC+ from the \verb+qpgraph+
package as follows:
...
the entire genome wide data, i.e., for all possible TF-TG pairs among the initial
$p=4\,205$ genes:

<<getthenet, echo=TRUE, results=verbatim>>=
pcc.gwide.estimates <- qpPCC(gds680.eset)
allTFs_i <- match(unique(filtered.regulon6.1[,"EgID_TF"]),featureNames(gds680.eset))
allTFsbyTGs <- as.matrix(expand.grid(allTFs_i,
                                     setdiff(1:length(featureNames(gds680.eset)),allTFs_i)))
allTFsbyTGs <- rbind(allTFsbyTGs,t(combn(allTFs_i, 2)))
summary(abs(pcc.gwide.estimates$R[allTFsbyTGs]))
@
...
<<getthenet, eval=FALSE, echo=FALSE, results=hide>>=
...
\begin{figure}
\centerline{\includegraphics[width=0.5\textwidth]{qpPCCdistbyTF}}
...
using the \verb+qpPrecisionRecall+ function from the \verb+qpgraph+ package.
...
<<getthenet, echo=TRUE, results=verbatim>>=
set.seed(12345)
rndcor <- matrix(runif(10000,min=-1,max=1), nrow=100, ncol=100)
...
postscript(file="qpPreRecComparison.eps", onefile=FALSE, paper="a4",
           pointsize=12, horizontal=FALSE, height=6, width=6)
@
<<getthenet, echo=TRUE, results=hide>>=
...
\includegraphics[width=0.45\textwidth]{qpPreRecComparison} &
\includegraphics[width=0.45\textwidth]{qpPreRecComparisonFullRecall} \\
(a) & (b)
\end{tabular}}
...
@
In the previous call to \verb+qpGraph+ we have set \verb+return.type="graphNEL"+
in order to get a \verb+graphNEL+ object which we can directly plot using the
\verb+Rgraphviz+ library as we will show later, but before that we want to
estimate the corresponding PACs for the interactions. First, we should see if
this is at all possible by calculating the size of the largest clique in this
undirected graph with the \verb+qpCliqueNumber+ function from the \verb+qpgraph+
package:
...
@
In order to build the final reverse engineered transcriptional regulatory network
from this 50\%-precision qp-graph we need to transform the undirected graph
\verb+g+ into a \verb+graphNEL+ directed graph with arrows pointing from TFs to
their targets. This can be accomplished by first setting the \verb+graphNEL+
object \verb+g+ that stores the qp-graph as a directed graph:

<<getthenet, echo=TRUE, results=verbatim>>=
edgemode(g) <- "directed"
@
This operation leaves \verb+g+ with directed edges between all genes that were
adjacent in the former undirected graph. Therefore, we should next remove those
directed edges that go from TG genes to TF genes:

<<getthenet, echo=TRUE, results=verbatim>>=
gTFvertices <- nodes(g)[!is.na(match(nodes(g), filtered.regulon6.1[,"EgID_TF"]))]

wrongEdges <- boundary(setdiff(nodes(g),gTFvertices),g)
wrongEdges <- wrongEdges[unlist(lapply(wrongEdges, length)) > 0]
wrongEdges <- matrix(unlist(sapply(names(wrongEdges),
                     function(x) t(cbind(x,wrongEdges[[x]])), USE.NAMES=FALSE)),
                     ncol=2, byrow=TRUE)
...
@
We can plot the network, which should look like the one in Figure~3, by loading
first the \verb+Rgraphviz+ library:

<<getthenet, echo=TRUE, results=verbatim>>=
library(Rgraphviz)
@
Next, get the genes forming connected components with more than 1 vertex
(i.e., those involved in edges) using the \verb+connComp+ function from the
\verb+graph+ package:

<<getthenet, echo=TRUE, results=verbatim>>=
gCc <- connComp(g)
gCc_gt1 <- gCc[(1:length(gCc))[unlist(lapply(gCc,length)) > 1]]
genesCcGt1 <- unique(unlist(gCc_gt1))
@
We extract the subgraph involving those genes, label them with their symbol
names and make TF genes look darker:

<<getthenet, echo=TRUE, results=verbatim>>=
gSub <- subGraph(genesCcGt1,g)

nodattr <- makeNodeAttrs(gSub,label=unlist(mget(nodes(gSub),org.EcK12.egSYMBOL)),
                         shape="ellipse",fixedsize=FALSE,fillcolor=grey(0.90))
gSubTFnodes <- nodes(gSub)[!is.na(match(nodes(gSub),filtered.regulon6.1[,"EgID_TF"]))]
nodattr$fillcolor[gSubTFnodes] <- grey(0.65)
@
Set different colors for edges representing RegulonDB interactions, novel
interactions, positive PACs and negative PACs:
...
edgattrs <- list(color=edgecolors)
@
We can finally do the plotting and put a legend as follows:

\begin{figure}
\label{fig:txregnet}
\centerline{\includegraphics[width=0.6\textwidth]{qpTRnet50pctpre}}
...
@
<<getthenet, echo=TRUE, results=hide>>=
rag <- plot(gSub, "twopi", nodeAttrs=nodattr, edgeAttrs=edgattrs, lwd=2,
            main="50%-precision qp-graph transcriptional regulatory network")
bb <- c(boundBox(rag)@upRight@x, boundBox(rag)@upRight@y)
legend(bb[1]*0.1,bb[2]*1.0,c("RegulonDB +","RegulonDB -","Novel +", "Novel -"),
       lwd=4, col=c("darkred","darkblue","orange","deepskyblue"))
@
<<getthenet, eval=FALSE, echo=FALSE, results=hide>>=
dev.off()
@
